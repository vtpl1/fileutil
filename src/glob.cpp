...
std::vector<fs::path> rlistdir(const fs::path& dirname, bool dironly) {
  ...
  for (auto&& name : names) {
    if (!is_hidden(name.string())) {
      ...
      std::copy(std::make_move_iterator(matched_dirs.begin()), std::make_move_iterator(matched_dirs.end()),
...
// directory.
std::vector<fs::path> glob2(const fs::path& dirname, [[maybe_unused]] const fs::path& pattern, bool dironly) {
  // std::cout << "In glob2\n";
  std::vector<fs::path> result{"."};
...
std::vector<fs::path> glob(const std::string& pathname) { return glob(pathname, false); }

std::vector<fs::path> rglob(const std::string& pathname) { return glob(pathname, true); }

std::vector<fs::path> glob(const std::vector<std::string>& pathnames) {
  std::vector<fs::path> result;
  for (const auto& pathname : pathnames) {
    auto matched_res = glob(pathname, false);
    std::copy(std::make_move_iterator(matched_res.begin()), std::make_move_iterator(matched_res.end()),
              std::back_inserter(result));
  }
  return result;
}
...
std::vector<std::string> glob(const std::string& pathname) {
  ...
  for (auto&& elem : glob::glob(pathname)) {
    ret.emplace_back(elem.string());
  }
  return ret;
}
...
std::vector<std::string> glob(const std::vector<std::string>& pathnames) {
  std::vector<std::string> ret;
  for (auto&& elem : glob::glob(pathnames)) {
    ret.emplace_back(elem.string());
  }
  return ret;
}

std::vector<std::string> rglob(const std::vector<std::string>& pathnames) {
  std::vector<std::string> ret;
  for (auto&& elem : glob::glob(pathnames)) {
    ret.emplace_back(elem.string());
  }
...
std::vector<std::string> glob(const std::initializer_list<std::string>& pathnames) {
  std::vector<std::string> ret;
  for (auto&& elem : glob::glob(pathnames)) {
    ret.emplace_back(elem.string());
  }
...
std::vector<std::string> rglob(const std::initializer_list<std::string>& pathnames) {
  std::vector<std::string> ret;
  for (auto&& elem : glob::rglob(pathnames)) {
    ret.emplace_back(elem.string());
  }
  return ret;
}
